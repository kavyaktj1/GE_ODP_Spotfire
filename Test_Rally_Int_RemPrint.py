...
output = open(os.path.join('export','rallyLog.txt'), 'ab')
output.write('Rall Log File')
output.write("v_version:" + str(v_version))
...
def get_csv_attachment_content(query_stmt):
    userstory_obj = rally.get('HierarchicalRequirement', query=query_stmt, order="FormattedID", pagesize=200, limit=400,
                              projectScopeDown=True)
    userstory = userstory_obj.next()
    context, augments = rally.contextHelper.identifyContext()
    for att in userstory.Attachments:
...
logFile.write('PyRal python script Log File\n')
